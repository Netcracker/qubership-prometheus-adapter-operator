...
HELM_FOLDER=charts/qubership-prometheus-adapter-operator
...
PROM_ADAPTER_OPERATOR_PKG=github.com/Netcracker/qubership-prometheus-adapter-operator

# The ldflags for the go build process to set the version related data.
GO_BUILD_LDFLAGS=\
	-s \
	-X $(PROM_ADAPTER_OPERATOR_PKG)/version.Revision=$(BUILD_REVISION) \
	-X $(PROM_ADAPTER_OPERATOR_PKG)/version.BuildUser=$(BUILD_USER) \
	-X $(PROM_ADAPTER_OPERATOR_PKG)/version.BuildDate=$(BUILD_DATE) \
	-X $(PROM_ADAPTER_OPERATOR_PKG)/version.Branch=$(BUILD_BRANCH) \
	-X $(PROM_ADAPTER_OPERATOR_PKG)/version.Version=$(VERSION)
...
CONTAINER_NAME="qubership-prometheus-adapter-operator"
...
prepare-charts:
	...
	mkdir -p "$(CRDS_DIR)/qubership-prometheus-adapter-operator"
...
archive-helm-chart:
	...
	# name like: qubership-prometheus-adapter-operator-14.20.0-chart.zip
...
archive-crds:
	...
	cp charts/qubership-prometheus-adapter-operator/crds/* "${BUILD_DIR}/_crds/qubership-prometheus-adapter-operator/"

	# Navigate to dir to avoid unnecessary directories in result archive\
	# name like: qubership-prometheus-adapter-operator-14.20.0-crds.zip
